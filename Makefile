...
ifneq ($(IOS_APP_GROUP),)
	...
	@mkdir -p build-ios
	@cd ios/ && xcodebuild -workspace SovrinConnector.xcworkspace/ -scheme SovrinConnector -sdk iphoneos -configuration Relase -parallelizeTargets -resultBundlePath ../build-ios/result -derivedDataPath ../build-ios/ CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO
	@cd build-ios/ && mkdir -p Payload && cp -R Build/Products/Release-iphoneos/SovrinConnector.app Payload/ && zip -r SovrinConnector-unsigned.ipa Payload/
	@mv build-ios/SovrinConnector-unsigned.ipa .
	@rm -rf build-ios/
	@rm -rf assets/override
	@ps -e | grep -i "cli.js start" | grep -iv grep | awk '{print $$1}' | xargs kill -9
...
unsigned-android: pre-run check-style prepare-android-build
	...
	@mv android/app/build/outputs/apk/app-unsigned-unsigned.apk ./SovrinConnector-unsigned.apk
	@ps -e | grep -i "cli.js start" | grep -iv grep | awk '{print $$1}' | xargs kill -9
