...
VCX_WORKDIR=~/dev/sovrin/sdk
# path of sovrinconnector repo on our machine
SOVRINCONNECTOR_WORKDIR=~/dev/sovrin/SovrinConnector
...
copy_wrapper(){
	cp -v ${VCX_WORKDIR}/vcx/wrappers/java/vcx/build/outputs/aar/vcx-release.aar ${SOVRINCONNECTOR_WORKDIR}/android/app/libs
}

build_wrapper_and_update_gradle_files(){
	AAR=$(build_wrapper | perl -nle 'print $& if m{(?<=vcx-1.0.0-)(.*)(?=.aar)}' | head -1 | awk '{print $1}' )
	echo ${AAR}
	REPLACE=$(cat ${SOVRINCONNECTOR_WORKDIR}/android/app/build.gradle | perl -nle "print $& if m{(?<=vcx:1.0.0-)(.*)(?=')}")
	echo " ${REPLACE} will be replaced with :: ${AAR} in build.gradle"
	sleep 10
	pushd ${SOVRINCONNECTOR_WORKDIR}/android/app/
...
uninstall_app(){
		adb uninstall org.sovrinconnector
		sleep 10
}
...
run_app(){
	...
	pushd ${SOVRINCONNECTOR_WORKDIR}
		...
		pushd android
			...
			adb shell pm grant org.sovrinconnector android.permission.SYSTEM_ALERT_WINDOW && \
			adb shell pm grant org.sovrinconnector android.permission.WRITE_EXTERNAL_STORAGE && \
			adb shell pm grant org.sovrinconnector android.permission.READ_EXTERNAL_STORAGE && \
			adb -s emulator-5554 shell am start -n org.sovrinconnector/org.sovrinconnector.MainActivity
